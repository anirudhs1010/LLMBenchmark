...
const judgePanels = [
  { label: 'Perplexity Sonar Ratings', key: 'sonar' as const },
  { label: 'Llama Ratings', key: 'llama' as const },
  { label: 'Perplexity R1-1776 Ratings', key: 'r1' as const },
];
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="space-y-8">
          {judgePanels.map(({ label, key }) => (
            <div key={key}>
              <h3 className="text-xl font-semibold text-center text-foreground mb-4">{label}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {ratingDetails.map(({ name, key: ratingKey, Icon, color, description }) => (
                  <Card key={`${key}-${name}`} className="shadow-lg hover:shadow-xl transition-shadow duration-300">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-lg font-medium">{name}</CardTitle>
                      <Icon className={`h-6 w-6 ${color}`} />
                    </CardHeader>
                    <CardContent>
                      <div className="text-4xl font-bold text-primary">{ratings[key][ratingKey]}</div>
